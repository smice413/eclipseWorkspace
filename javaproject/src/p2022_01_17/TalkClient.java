...
//    String name = "gemini"; //대화명
    ...
    public void connectServer( String id, String address ) {
		...
		// 클라이언트 자체도 쓰레드로 구동시킴
		Thread t = new Thread( this );//this는 자기자신을 호출하는 것
		t.start(); // run() 메소드 호출
    }// connectServer() end
		...
		while( true ) {
			...
			try {
				...
				while(st.hasMoreTokens()){
					msgPlus += st.nextToken();// 혹시나 파싱용으로/가 되어있는데 메세지상/가 여러번 들어가 있으면 메세지가 잘리는 것을 방지하기 위한 코드
				}
					view.append(msgPlus + "\n");// 혹시나 파싱용으로/가 되어있는데 메세지상/가 여러번 들어가 있으면 메세지가 잘리는 것을 방지하기 위한 코드
			} catch( Exception e ) {
