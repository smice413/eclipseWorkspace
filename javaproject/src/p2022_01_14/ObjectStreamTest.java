...
public class ObjectStreamTest {
	...
	try {
	    ...
	    ObjectOutputStream oos = new ObjectOutputStream( fos );//객체 직렬화시키는 클래스

	    // write()를 이용해 객체를 file에 저장
	    oos.writeObject( gemini ); //객체가 person.dat에 저장됨
	...
	try {
	    ...
	    // 파일(person.dat)에 저장된 객체를 모두 읽어 올 때까지 반복
  	    while(( o = ois.readObject() ) != null ) {//ois.readObject() 입력한 객체를 불러오는 메소드이며 o변수에 저장됨.

			// if( 변수 instanceof 클래스명) 
			//  : 변수가 이 클래스의 instance인지 아닌지를 체크함.
			// 만약 읽어온 객체가 PersonInforamtion 객체이면
			if( o instanceof PersonInformation ) { //instanceof : o안에 저장된 gemini가 personinformation에서 만든 객체가 맞는지 확인하는 메소드 true면 진행 
				System.out.print(((PersonInformation)o).getName() + " : ");//o가 Object형이기 때문에 다운캐스팅
				...
				System.out.println( o.toString() );//toString은 String으로 출력됨.
			}
	    }//while end
	} catch ( Exception e ) {
	}
