...
String sql="update model2 set board_re_seq=board_re_seq+1 ";
	   sql+=" where board_re_ref=? and board_re_seq > ?";
	   	...
	   	sql="insert into model2 values(model2_seq.nextval,?,?,?,?,?,?,?,?,?,sysdate)";
	   		...
	   		pstmt.setString(3, board.getBoard_subject());
	   		pstmt.setString(4, board.getBoard_content());
	   		pstmt.setString(5, "");	// board_file 첨부파일은 댓글로 작성하지 않기 때문에 null값 준것
	   		pstmt.setInt(6, re_ref);  // board_re_ref
	   		pstmt.setInt(7, re_lev+1);// board_re_lev  
	   		pstmt.setInt(8, re_seq+1);// board_re_seq +1은 부모글 밑에 댓글이 들어가도록 하는 것
