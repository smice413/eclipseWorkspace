...
function id_check(){
    ...
    $.ajax({
        ...
        data: {"memid":memid, "key":"1234"},  //{"memid":memid, "key":"1234"} 이렇게  콤마하고 키값 : value값을 넣으면 여러개의 데이터를 서버에 전달할 수 있다.
...
function validate_userid(memid){
  var pattern= new RegExp(/^[a-z0-9_]+$/);//정규표현식 RegExp(/^[a-z0-9_]+$/); 또는 "/^[a-z0-9_]+$/" 생성함
  //영문 소문자,숫자 ,_가능,정규표현식
