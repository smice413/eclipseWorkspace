...
public class BoardDBBean {
	...
	public int insert(BoardDataBean board) {
		...
		try {
			...
			pstmt.setInt(5, board.getReadcount()); //readcount가 int형이기때문에 0으로 초기값설정됨
...
//				result = rs.getInt(1);
	...
	public int update(BoardDataBean board) {
		...
		try {
			...
			result = pstmt.executeUpdate(); //SQL문 실행. 정상적 실행 1리턴됨.
		...
		try {
			...
			if(upload.getUpload() != null) {	// 첨부 파일이 있으면
				...
				for(int i=0; i<f.length; i++) {
					...
					// upload 디렉토리에 저장된 파일 중에서 DB에 저장된 파일
					if(f[i].getName().equals(upload.getUpload())) { //upload.getUpload(): deletePro.jsp에서 old매개변수(DB데이터)로 값을 받은 것
						f[i].delete();		//첨부파일 삭제				//f[i].getName() : 배열에 저장된 파일 이름
					}
				}
			}
		}catch(Exception e) {
